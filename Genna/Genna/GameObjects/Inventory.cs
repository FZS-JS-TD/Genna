...
namespace Genna.GameObjects
{
    ...
    public class Inventory
    {
        ...
        public Dictionary<Vector2, Item> InvDict
        {
            get { return _invDict; }
            set { _invDict = value; }
        }
        ...
        public Inventory(Zanaj pZan)
        {
            ...
            _smallInstrucFont = game.Content.Load<SpriteFont>("Fonts/smallInstrucFont");
        }
        ...
        public void RemoveItem(int x, int y)
        {
            ...
            if (_invDict.ContainsKey(v))
            {
                if (_invDict[v].Equipped == true && _invDict[v].Type > 4 && _invDict[v].Type < 13)
                {
                    zan._EqWeapon = new Weapon(null, 1, 5, "Peashooter", 50, 5, 20, 1, 1, 200);
                }
                else if (_invDict[v].Equipped == true && _invDict[v].Type == 2)
                {
                    zan._EqBoots = new Boots(null, 1, 2, "Original Boots", 20, 10, 0);
                }
                else if (_invDict[v].Equipped == true && _invDict[v].Type == 3)
                {
                    zan._EqGloves = new Gloves(null, 1, 3, "Original Gloves", 20, 10, 0);
                }
                else if (_invDict[v].Equipped == true && _invDict[v].Type == 4)
                {
                    zan._EqBreastplate = new Breastplate(null, 1, 4, "Original Breastplate", 30, 20, false, 0, 0);
                    zan._eqBreastplateIndex = new Vector2(-1, -1);
                }
                _invDict.Remove(v);
                _invSlot[x, y] = null;
            }
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            spriteBatch.DrawString(_smallInstrucFont, "E --- Equip", new Vector2(610, 210), Color.White);
            spriteBatch.DrawString(_smallInstrucFont, "S --- Sell/Remove", new Vector2(610, 230), Color.White);
            spriteBatch.DrawString(_smallInstrucFont, "I --- Exit Inventory", new Vector2(610, 250), Color.White);
            spriteBatch.DrawString(_smallInstrucFont, "Character Stats:", new Vector2(640, 280), Color.White);
            ...
            Item cursorItemAtIndex = this[(_cursorRect.X / 50), (_cursorRect.Y / 50)];
        ...
        public void Update()
        {
            ...
            if (PressedOnlyOnce(Keys.S))
            {
                Item i = this[(_cursorRect.X / 50), (_cursorRect.Y / 50)];
                moneyAmount += i.Value;
            ...
            if (PressedOnlyOnce(Keys.M))
            {
                Item i = this[(_cursorRect.X / 50), (_cursorRect.Y / 50)];
                if (i.Type == 0)
                {
                    zan._EqBreastplate.PlusPoisonResist += 10;
                    Vector2 v = zan._eqBreastplateIndex;
                    zan._inventory.InvSlot[(int)v.X, (int)v.Y].PlusPoisonResist += 10;
